...
const PrimVisualizer = ({ data, steps, currentStep, totalSteps, isPlaying, onStop, onNext, onPrev, onRestart }) => {
  ...
  useEffect(() => {
    ...
    if (stepData.graph) {
      nodes.forEach(fromNode => {
        ...
        neighbors.forEach(neighborObj => {
          ...
          if (isProcessingNeighbors && (stepData.operation === 'add_edge' || stepData.operation === 'update_queue')) {
    ...
    nodes.forEach(node => {
      ...
      // In queue nodes
        ...
        fillColor = "#DBEAFE"; // Very light blue for in queue
        strokeColor = "black";
        textColor = "black";
      }
  ...
  const getOperationDescription = (stepData) => {
    ...
    if (operation === 'start') {
      return `Starting Prim's algorithm from node ${stepData.startNode}`;
    } else if (operation === 'select_node') {
      return `Selecting node ${stepData.currentNode} with minimum edge`;
    } else if (operation === 'add_edge') {
      return `Adding edge ${stepData.addEdge.from}-${stepData.addEdge.to} with weight ${stepData.addEdge.weight} to MST`;
    } else if (operation === 'update_queue') {
      return `Updating edge queue with new edges from node ${stepData.currentNode}`;
          ...
          <div className={`text-center p-2 bg-white border border-gray-200 ${isFullscreen ? 'hidden' : ''}`}>
            ...
            {currentStepData && (
              <div className="mt-2 text-xs">
                ...
                {currentStepData.inQueue && currentStepData.inQueue.length > 0 && (
                  <p className="text-blue-400">
                    In Queue: [{currentStepData.inQueue.join(', ')}]
                  </p>
                )}
              </div>
            )}
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 text-xs">
                      ...
                      {step.inQueue && step.inQueue.length > 0 && (
                        <p className="text-blue-400">
                          In Queue: [{step.inQueue.join(', ')}]
                        </p>
                      )}
                    </div>
                  </div>
