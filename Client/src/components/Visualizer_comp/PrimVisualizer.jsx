...
const PrimVisualizer = ({ data, steps, currentStep, totalSteps, isPlaying, onStop, onNext, onPrev, onRestart }) => {
  ...
  const downloadStepsAsPDF = async () => {
    ...
    // Add title
    doc.setFontSize(22);
    doc.text('Prim\'s Algorithm Visualization Steps', 148.5, 15, null, null, 'center');
    ...
    for (let index = 0; index < steps.length; index++) {
      ...
      if (step.graph || step.operation) {
        ...
        drawGraphInPDF(doc, step.graph, step.mst, x, y, 0, null, null, scale, step);
      } else {
  ...
  const drawGraphInPDF = (doc, graph, mst, offsetX, offsetY, level = 0, parentX = null, parentY = null, scale = 1, stepData = null) => {
    if (!graph) return;
    ...
    nodes.forEach((node, index) => {
      ...
      neighbors.forEach(neighborObj => {
        ...
        const isMSTEdge = mstEdgeSet.has(`${node}-${neighbor}`);
        ...
        doc.setLineWidth(isMSTEdge ? 1.5 * scale : 0.5 * scale);
    ...
    nodes.forEach((node, index) => {
      ...
      if (stepData) {
        if (stepData.visited && stepData.visited.includes(node)) {
          fillColor = [16, 185, 129]; // green-500
          strokeColor = [5, 150, 105]; // green-600
        } else if (stepData.currentNode === node) {
  ...
  const getNodeStyle = (stepData, nodeValue) => {
    ...
    if (stepData.visited && stepData.visited.includes(nodeValue)) {
      baseStyle += "bg-green-500 text-white border-green-600";
    } else if (stepData.currentNode === nodeValue) {
  ...
  const getOperationDescription = (stepData) => {
    ...
    if (operation === 'start') {
      return `Starting Prim's algorithm from node ${stepData.currentNode}`;
      ...
      return `Skipping edge ${stepData.currentEdge.from}-${stepData.currentEdge.to} (destination already visited)`;
    } else if (operation === 'update_queue') {
      return `Updating edges queue after adding node ${stepData.currentEdge.to}`;
    } else if (operation === 'complete') {
      return 'Prim\'s algorithm complete!';
    } else {
      return 'Processing...';
    }
  };
  ...
  const renderGraphNode = (graph, mst, stepData) => {
            ...
            return (
              <g key={`${node}-${neighbor}`}>
                <line
                  ...
                  x2={x2}
                  y2={y2}
                  stroke={isMSTEdge ? "#10B981" : "#9CA3AF"} // green-500 for MST, gray-400 for others
                  strokeWidth={isMSTEdge ? "3" : "2"}
                  className={isMSTEdge ? "stroke-green-500" : "stroke-gray-400"}
                />
                {/* Edge weight */}
                <text
                  x={(x1 + x2) / 2}
            ...
            {currentStepData ? (
              <div className="w-full min-h-[400px] flex items-center justify-center overflow-auto">
                <svg width="100%" height="500" className="border border-gray-200 rounded min-w-[600px]" viewBox="0 0 600 500">
                  {renderGraphNode(currentStepData.graph, currentStepData.mst, currentStepData)}
                </svg>
              </div>
            ) : (
                      ...
                      {step.graph || step.operation ? (
                        <div className="w-full min-h-[200px] overflow-auto">
                          <svg width="100%" height="300" className="border border-gray-200 rounded min-w-[400px]" viewBox="0 0 400 300">
                            {renderGraphNode(step.graph, step.mst, step)}
                          </svg>
                        </div>
                      ) : (
